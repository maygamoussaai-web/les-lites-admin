/**
 * Resultats periodiques : une case par periode ouverte/fermee,
 * moyenne live (formules modele) ou moyenne validee du bulletin.
 */
...
export function PeriodResultsCard({
  ...
  const { loading, periods, grades, activePeriod } = useClassGrades(classId ?? "", !!classId);
    ...
    return sorted.map((p) => {
      ...
      const validatedAvg =
        card?.general_average !== null && card?.general_average !== undefined
          ...
          : null;
      const liveAvg = studentPeriodAverage(
        grades.filter((g) => g.period_id === p.id),
        studentId,
      );
      const average = validatedAvg ?? liveAvg;
      const isActive = activePeriod?.id === p.id;
      const isClosed = p.ended_at !== null;
      return { period: p, average, isActive, isClosed, validated: validatedAvg !== null };
    });
  }, [periods, grades, cards, studentId, activePeriod?.id]);

  const annual =
    rows.filter((r) => r.average !== null).length > 0
      ? rows.filter((r) => r.average !== null).reduce((a, r) => a + (r.average as number), 0) /
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-3">
        <CardTitle className="text-base">Resultats periodiques</CardTitle>
      ...
      <CardContent>
        {!classId ? (
          <p className="text-xs text-muted-foreground">Eleve non assigne a une classe.</p>
        ) : loading ? (
          <p className="text-xs text-muted-foreground">Chargement…</p>
        ) : rows.length === 0 ? (
          <p className="text-xs text-muted-foreground">Aucune periode creee pour cette classe.</p>
        ) : (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
            ...
            <div className="rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-center">
              <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Annuelle</p>
