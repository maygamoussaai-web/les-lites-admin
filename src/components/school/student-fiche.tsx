...
import {
  ArrowLeft, Pencil, ArrowRightLeft, Trash2, ShieldAlert, Receipt, Wallet,
  IdCard, Library, Archive, RotateCcw,
} from "lucide-react";
...
import { StudentGradesCard } from "@/components/school/student-grades-block";
...
export function StudentFichePage() {
  ...
  const lastEnrollment = [...(data.enrollments ?? [])]
  ...
  const restoreStudent = async () => {
    ...
    try {
      ...
      await writeAudit("update", "students", student.id, { restored: true });
      qc.invalidateQueries({ queryKey: ["students"] });
      toast.success("Élève restauré — il réapparaît dans les listes actives");
    } catch (e) {
      toast.error(describeError(e, "Restauration impossible"));
    } finally {
  ...
  return (
    <>
      ...
      {isArchived && (
        ...
        >
          <p className="text-amber-950 dark:text-amber-100">
            ...
            <strong>Dossier archivé</strong>
            {student.archived_at ? ` le ${formatDate(student.archived_at)}` : ""}
            {" "}— lecture seule. Scolarité, notes et bibliothèque restent accessibles.
          </p>
          <Button
            size="sm"
            variant="outline"
            ...
            onClick={() => void restoreStudent()}
          >
            <RotateCcw className="mr-1.5 h-4 w-4" />
            {restoreBusy ? "Restauration…" : "Restaurer"}
          </Button>
        </div>
      )}
      ...
      <PageHeader
        ...
        description={
          isArchived
            ? `${establishment?.name ?? "—"} · ${lastEnrollment?.class_name ?? klass?.name ?? "Classe inconnue"} · données conservées`
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-xl border border-border/70 bg-card p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            {klass?.name ?? lastEnrollment?.class_name ?? "Classe non assignée"} · {establishment?.name ?? "—"}
          </p>
          {isArchived && (
            <Badge variant="outline" className="mt-1.5 text-muted-foreground">Archivé</Badge>
          )}
        </div>
          ...
          <CardContent className="space-y-2.5 text-sm">
            {enrollment ? (
              ...
              <p className="text-muted-foreground">
                {isArchived
                  ? "Aucune période active — consultez l'historique via Détail."
          ...
          <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Bulletins générés, actes, photos et autres pièces.
              </p>
            </div>
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" className="press">
                <Trash2 className="mr-1.5 h-4 w-4" /> Archiver
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Archiver {student.first_name} {student.last_name} ?</AlertDialogTitle>
                <AlertDialogDescription>
                  L'élève quitte les listes actives et passe dans Archives. Notes, bulletins et scolarité sont conservés.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={async () => {
                    ...
                    archive.mutate(student.id, {
                      onSuccess: () => navigate({ to: "/archives" }),
                    });
                  }}
                >
                  Archiver
