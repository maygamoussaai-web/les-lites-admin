...
import { useActiveReportTemplate, gradeNaturesFromMapping } from "@/lib/report-template";
...
export function ClassPage() {
  ...
  const { template: activeTemplate, loading: templateQueryLoading } = useActiveReportTemplate(classId);
  ...
  useEffect(() => {
    ...
    if (activeTemplate) {
      setTemplateBuffer(activeTemplate.buffer);
      setTemplateMapping(activeTemplate.mapping);
      setTemplateScale(activeTemplate.scale);
    ...
    setTemplateError(
      "Aucun modèle Excel actif — les moyennes ne peuvent être calculées (formules du modèle requises).",
    );
    setTemplateReady(true);
  }, [activeTemplate, templateQueryLoading]);
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      try {
        ...
        if (cancelled) return;
        const requiredNatures = gradeNaturesFromMapping(templateMapping);
        ...
        for (const s of classStudents) {
          ...
          const weakSubjects = Object.entries(model.subjectAverages)
            .filter(([name, avg]) => {
              ...
              const sub = subjectsQuery.data.find((x) => x.name === name);
              if (!sub) return false;
              const list = gradesQuery.data.filter(
                (g) => g.student_id === s.id && g.subject_id === sub.id,
              );
              return subjectNotesComplete(list, requiredNatures);
            })
            .map(([name]) => name);
        ...
        for (const s of classStudents) {
          ...
          for (const [name, avg] of Object.entries(model.subjectAverages)) {
            ...
            const sub = subjectsQuery.data.find((x) => x.name === name);
            if (!sub) continue;
            const studentList = gradesQuery.data.filter(
              (g) => g.student_id === s.id && g.subject_id === sub.id,
            );
            if (!subjectNotesComplete(studentList, requiredNatures)) continue;
    ...
    <>
      ...
      {templateBuffer && templateMapping && (
        <div className="rounded-md border border-success/30 bg-success/10 px-3 py-2 text-xs text-success">
          Moyennes uniquement via les formules du modele Excel.
        </div>
      )}
      {statsLoading && <p className="text-xs text-muted-foreground">Calcul des moyennes…</p>}
      ...
      <NoteEntryDialog
        ...
        existingGrades={gradesQuery.data}
      />
      ...
      <AlertDialog open={renewOpen} onOpenChange={setRenewOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              La période en cours sera clôturée. Les notes déjà saisies restent accessibles dans l'historique.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
