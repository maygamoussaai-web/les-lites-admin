    ...
    (async () => {
      ...
      try {
        ...
        if (!tpl?.file_path || !tpl.mapping) {
          if (!cancelled) {
            setTemplateError(
              "Aucun modele Excel actif — les moyennes ne peuvent etre calculees (formules du modele requises).",
            );
      ...
      <CardContent className="space-y-2">
        {rows.map((r) => (
          <div key={r.student.id} className="flex items-center justify-between gap-2 text-sm">
            <Link to="/eleves/$studentId" params={{ studentId: r.student.id }} className="font-medium hover:underline">
              {r.student.last_name} {r.student.first_name}
            </Link>
            <Badge variant={tone === "destructive" ? "destructive" : "default"}>{r.average.toFixed(2)}</Badge>
          </div>
        ))}
      </CardContent>
    </Card>
