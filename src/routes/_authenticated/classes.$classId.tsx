    ...
    (async () => {
      ...
      try {
        ...
        if (!tpl?.file_path || !tpl.mapping) {
          if (!cancelled) {
            setTemplateError("Aucun modele Excel actif — moyennes calculees a partir des notes saisies.");
  ...
  useEffect(() => {
    ...
    if (!templateBuffer || !templateMapping) {
      ...
      for (const s of classStudents) {
        const avg = studentPeriodAverage(gradesQuery.data, s.id);
        if (avg === null) continue;
        const bySubject = groupGradesBySubject(gradesQuery.data, s.id);
        const weakSubjects: string[] = [];
        for (const [sid, list] of bySubject) {
          const a = subjectAverage(list);
          if (a !== null && a < PASS_THRESHOLD) {
            const sub = subjectsQuery.data.find((x) => x.id === sid);
            weakSubjects.push(sub?.name ?? "Matiere");
          }
        }
      ...
      const classAverage = withAvg.length
        ...
        : null;
      const highest = withAvg[0] ?? null;
      const lowest = withAvg.length ? withAvg[withAvg.length - 1]! : null;
      const subjectAverages = subjectsQuery.data
        .map((sub) => {
          const list: number[] = [];
          for (const s of classStudents) {
            const gs = groupGradesBySubject(gradesQuery.data, s.id).get(sub.id) ?? [];
            const a = subjectAverage(gs);
            if (a !== null) list.push(a);
          }
          return { subject: sub, avg: list.length ? list.reduce((x, y) => x + y, 0) / list.length : null };
        })
        .filter((x): x is { subject: ClassSubject; avg: number } => x.avg !== null);
      const bestSubject = subjectAverages.length
        ? subjectAverages.reduce((a, b) => (b.avg > a.avg ? b : a))
        : null;
      const worstSubject = subjectAverages.length
        ? subjectAverages.reduce((a, b) => (b.avg < a.avg ? b : a))
        : null;
    ...
    <>
      ...
      {templateBuffer && templateMapping && (
        <div className="rounded-md border border-success/30 bg-success/10 px-3 py-2 text-xs text-success">
          Moyennes via formules du modele Excel (repli notes si modele sans moyenne).
        </div>
      )}
      {statsLoading && (
      ...
      <CardContent className="space-y-2">
        {rows.map((r) => (
          <div key={r.student.id} className="flex items-center justify-between gap-2 text-sm">
            ...
            <Badge variant={tone === "destructive" ? "destructive" : "default"}>
              {r.average.toFixed(2)}
            </Badge>
          </div>
        ))}
