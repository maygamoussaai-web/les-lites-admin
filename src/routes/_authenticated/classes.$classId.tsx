...
function Page() {
  ...
  const stats: ClassStats | null = useMemo(() => {
    if (!latestPeriod || !templateReady) return null;
    ...
    if (warnings.length) console.warn("Formules modele (resultats classe)", warnings);
    ...
    <>
      ...
      {templateError && (
        <div className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {templateError} Importez un modele de bulletin pour afficher les moyennes (formules Excel).
        </div>
      )}
      {templateBuffer && templateMapping && (
        <div className="rounded-md border border-success/30 bg-success/10 px-3 py-2 text-xs text-success">
          Moyennes calculees avec les formules du modele Excel de la classe (pas les formules de l'app).
        </div>
      )}
...
function ClassReportsSection({
  ...
  const download = async (report: ClassReport) => {
    ...
    downloadBlob(await res.blob(), `Rapport-${className}-P${period?.period_number ?? ""}.pdf`);
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {activeReports.length === 0 ? (
          <p className="text-sm text-muted-foreground">Aucun rapport actif (disponibles 48h après génération).</p>
        ) : (
...
function renderClassReportCanvas({
  ...
  ctx.fillText(`Moyenne de classe : ${stats.classAverage != null ? stats.classAverage.toFixed(2) : "—"}`, 60, y);
  y += 40;
  ctx.fillText(`Réussite : ${stats.passing.length}/${stats.withAvg.length}`, 60, y);
  y += 40;
  if (stats.highest) {
    ctx.fillText(`Premier : ${stats.highest.student.last_name} ${stats.highest.student.first_name} (${stats.highest.average.toFixed(2)})`, 60, y);
    y += 36;
  }
  if (stats.lowest) {
    ctx.fillText(`Dernier : ${stats.lowest.student.last_name} ${stats.lowest.student.first_name} (${stats.lowest.average.toFixed(2)})`, 60, y);
  }
  return canvas;
}
